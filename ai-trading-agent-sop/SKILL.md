...
version: 3.2.0
metadata:
 author: Brody Goodwin
 account: Robinhood agentic trading (dedicated account "Agentic", ••••6751 / 822956751, limited-margin account type; not yet funded)
...
- **No options (was Rule 3).** Not supported by Robinhood's agentic-trading beta anyway (per the platform-constraints section below). Kept as a rule regardless.
...
**2026-09-14, later same day — crypto ban lifted (was Rule 2).** Brody asked specifically for crypto to be back in scope if Claude judges a specific coin better-positioned for gains than the day's equity candidates. Correcting an error in this amendment's first version: crypto trading IS supported by this MCP connection (`place_crypto_order`, `get_crypto_quotes`, `get_crypto_positions`, etc. are all available, and the account carries crypto buying power) — the "not supported by the platform" reasoning given earlier was wrong. Crypto also doesn't carry the debt-liability risk margin does (a cash crypto buy is bounded the same way a stock buy is), so this one is lifted on Brody's specific request. Applying the same liquidity/manipulation floor as penny stocks: stick to Robinhood-listed, reasonably liquid, large-cap cryptocurrencies (e.g. BTC, ETH, and other major listed coins) — not obscure microcap tokens, which are if anything worse for manipulation/liquidity risk than microcap stocks. No crypto margin/leverage (same absolute ban as equities).

Everything else is now discretionary: no cluster/signal requirement to originate a buy or sell; no fixed earnings-window blackout; no fixed chase-cap; position sizing is Claude's judgment call rather than a fixed ≤35% ceiling (though for a $50 account, going all-in on one name rarely buys anything worth the lost diversification — reasonable sizing judgment still applies, not maximum concentration by default). In scope: long U.S. equities/ETFs on NYSE/Nasdaq, and now long, liquid, large-cap crypto — whichever Claude judges best-positioned for the day's single trade.
...
18. **Trade event contracts / prediction markets directly.** Signal C is a DATA LAYER only - it informs long ETF positions. Direct event-contract trading requires an explicit rule change from you AND platform support; until then it is forbidden.
...
- **One-tap kill switch** in the app (our manual stop). **No documented hard $ cap** - rely on small, deliberate funding + account isolation + kill switch, not a platform limit.
- **Investor-profile gate before the 2nd trade (known platform behavior, per the SOP's original author).** Robinhood blocks the SECOND trade on an agentic account (HTTP 400, "answer some questions about your investing goals") until the account's investor-profile questionnaire is completed. The FIRST trade goes through; #2 onward fail until the profile is done at `applink.robinhood.com/investment_profile?account_number=<acct>&context=second_trade`. One-time gate per account - once you complete it, subsequent trades are unblocked. Not yet verified on this account; confirm live if/when it comes up.
...
- **The tradable account is the dedicated account nicknamed "Agentic" (`agentic_allowed: true`); verified 2026-09-14 as ••••6751 (822956751).** Its Robinhood account type is `limited_margin`, not `cash` - Hard Rule 5 (no leverage/margin/short selling; cash-account *behavior* only) still applies regardless of the account's platform classification: never place a margin trade or short sale on it. Not yet funded (current balance $0). The other two accounts on file (••••3991 margin-default, ••••6422 cash nicknamed "Brody G") are both `agentic_allowed: false` - read-only to the agent, never traded. Always confirm via `get_accounts` before any order.
...
7. **Pre-register rules, then don't drift.** This SOP is the frozen spec. Mid-session rule-bending because a trade "looks good" is exactly how the experiment's losers were built. Rule changes happen via you, in writing, in this file.

## Approval workflow - human-in-the-loop, then autonomous

Start with approval, learn from answers, then graduate.
...
- **Phase 3 - Autonomous** (only when ALL met): ≥20 logged decisions, ≥80% approval rate over the last 10, AND you say "go autonomous." Then execute clear-cut trades without asking but always post-notify, keep logging, and fall back to asking on any edge case.

**Kill switch:** you can disconnect the agent in the Robinhood app instantly, or say "pause" / "approval mode" to force Phase 1.
...
## Stop and ask before proceeding, no matter what
